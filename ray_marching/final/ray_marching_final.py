...
def main():
    ...
    vertex_shader = compileShader(read_shader_file("ray_marching.vs"), GL_VERTEX_SHADER)
    fragment_shaders = [
        compileShader(read_shader_file(f"ray_marching_{shader_name}.fs"), GL_FRAGMENT_SHADER)
        for shader_name in ("base", "shadows", "csg", "soft", "deformations")
    ]

    shader_programs = [
        compileProgram( vertex_shader, fragment_shader)
        for fragment_shader in fragment_shaders
    ]

    bind_vertices(vertices)

    shader_program = shader_programs[0]
    ...
    while not glfw.window_should_close(window):
        ...
        if glfw.get_key(window, glfw.KEY_1) == glfw.PRESS:
            shader_program = shader_programs[1]
            glUseProgram(shader_program)
        if glfw.get_key(window, glfw.KEY_2) == glfw.PRESS:
            shader_program = shader_programs[2]
            glUseProgram(shader_program)
        if glfw.get_key(window, glfw.KEY_3) == glfw.PRESS:
            shader_program = shader_programs[3]
            glUseProgram(shader_program)
            mode = 1
        ...
        if glfw.get_key(window, glfw.KEY_6) == glfw.PRESS:
            shader_program = shader_programs[4]
            glUseProgram(shader_program)
            mode = 1
        ...
        load_uniform_to_shader(shader_program, glfw.get_time(), "u_time", 1)
        load_uniform_to_shader(shader_program, smooth, "u_smooth", 1)
        load_uniform_to_shader(shader_program, sphere_z, "u_sphere_z", 1)
        load_uniform_to_shader(shader_program, sphere_y, "u_sphere_y", 1)
        load_uniform_to_shader(shader_program, sphere_r, "u_sphere_r", 1)
        load_uniform_to_shader(shader_program, mode, "u_mode", 1)
